...
fun CalendarView(
	...
	calendarAnimator: CalendarAnimator = CalendarAnimator(config.value.startDate),
	...
	val previousMonthDays by remember { mutableStateOf(calculateVisibleDaysOfPreviousMonth(yearMonth.value)) }
	...
	Column(
		...
		if (config.value.showHeader) {
			if(config.value.headerCanScroll){
